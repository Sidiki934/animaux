 ...
 include_once 'db_connect.inc.php';
    $sql = 'SELECT p.fname AS anim, g.nom AS gener, c.fname AS client, r.regionName AS reg, p.gender AS gend, p.photo AS tof, p.pet_id AS pet_id FROM pet p JOIN contacts c ON p.cli_id=c.cli_id JOIN generique g ON p.gen_id=g.gen_id JOIN region r ON p.reg_id=r.reg_id WHERE r.regionName = ?';

    $data = $conn->prepare($sql);
    $data->execute(array($_GET['region']));
   /* $data = $pdo->query($sql);*/
    $html = '';
